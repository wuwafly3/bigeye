...
## site.json — 站点配置（图床 / CDN）

图片量大时可以把素材迁到图床，数据文件里的相对路径不用改：

```jsonc
{
  "assetBase": ""   // 图床根地址，如 "https://img.example.com/agarchive"
}
```

- `assetBase` 为空：素材从本站 `public/` 加载（默认）。
- 填入图床根地址后：所有**相对路径**素材（角色图/插画/敌人图/首页背景/
  MMD 模型/弥弥尔素材）自动改从 `assetBase/<相对路径>` 加载，目录结构
  与 `public/` 保持一致即可整体迁移。
- 单条数据也可以直接填**完整 URL**（`https://…`），优先级高于 assetBase，
  适合逐步迁移或个别图挂外链。

注意事项：
1. 选择**支持外链直连**的图床（无防盗链限制，如对象存储 OSS/COS/R2 +
   自有域名；微博/微信图床有防盗链，不可用）。
2. MMD 模型走图床时，图床需配置 **CORS 允许本站域名**（three.js 以
   fetch 方式加载模型与贴图）。
3. 插画建议同时准备缩略图以节省流量（瀑布流用小图、灯箱用原图，
   可在 image 字段直接填不同尺寸的外链）。
